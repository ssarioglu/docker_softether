...
if [[ $USERS ]]
then
  echo '# <use the password specified>'
else
  : ${USERNAME:=user$(cat /dev/urandom | tr -dc '0-5' | fold -w 4 | head -n 1)}
  echo \# ${USERNAME}
  ...
  then
    ...
    PASSWORD=$(cat /dev/urandom | tr -dc '0-5' | fold -w 20 | head -n 1 | sed 's/.\{4\}/&./g;s/.$//;')
    echo \# ${PASSWORD}
  fi
fi
